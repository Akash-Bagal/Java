...
public class StaffController {
    ...
    // Get all staff
    @GetMapping("/all")
    public ResponseEntity<List<Classroom>> getAllStaff() {
        List<Classroom> staffList = staffService.getAllStaff();
    ...
    public ResponseEntity<?> getStaffByRole(@Valid @PathVariable String role) {
        List<Classroom> staffList = staffService.getStaffByRole(role);
    ...
    public ResponseEntity<?> getStaffById(@PathVariable int id) {
        Classroom staff = staffService.getStaffById(id);
        if (staff == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
    ...
    public ResponseEntity<?> getStaffByExperience(@PathVariable int exp) {
        List<Classroom> staffList = staffService.getStaffAboveExp(exp);
        if (staffList.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
    ...
    // Add a new staff
    @PostMapping("/add")
    public ResponseEntity<String> addStaff(@Valid @RequestBody Classroom staff) {
    ...
    @PutMapping("/update")
    public ResponseEntity<String> updateStaff(@Valid @RequestBody Classroom staff) {
